...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "👑 *Lệnh Admin:*\n"
        "👤 /tenme @username Tên — Đặt tên nhân viên\n"
        "⚙️ /setca HH MM — Set giờ ca chuẩn\n"
        "🗑 /xoalichsu — Xóa toàn bộ lịch sử\n"
        "🗑 /xoalichsu DD/MM — Xóa theo ngày"
    )
...
async def ten_me(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Chỉ admin đặt tên hiển thị cho nhân viên"""
    user = update.effective_user
    ...
    if len(context.args) < 2:
        await update.message.reply_text(
            "❌ Dùng: `/tenme @username Tên`\nVí dụ: `/tenme @anhteo Anh Tèo`",
            parse_mode="Markdown"
        )
        return
    # Tìm user theo username trong DB
    username_target = context.args[0].replace("@", "")
    ten_moi = " ".join(context.args[1:])
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT user_id FROM trang_thai WHERE username LIKE ?", (f"%{username_target}%",))
    row = c.fetchone()
    conn.close()
    if not row:
        await update.message.reply_text(f"❌ Không tìm thấy @{username_target}! Người đó cần lên ca trước.")
        return
    set_ten_hien_thi(row[0], ten_moi)
    await update.message.reply_text(
        f"✅ Đã đặt tên *@{username_target}* thành: *{ten_moi}*",
        parse_mode="Markdown"
    )

async def set_ca(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Chỉ admin set giờ lên ca và xuống ca chuẩn"""
...
async def xoa_lich_su(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Chỉ admin xóa lịch sử chấm công"""
    user = update.effective_user
    if user.id != MANAGER_ID:
        await update.message.reply_text("❌ Chỉ admin mới dùng được lệnh này!")
        return
    ...
    if not context.args:
        # Xóa toàn bộ
        c.execute("DELETE FROM chamcong")
        c.execute("DELETE FROM trang_thai")
        conn.commit()
        conn.close()
        await update.message.reply_text(
        ...
        # Xóa theo ngày DD/MM
        ngay_input = context.args[0]
        try:
            parts = ngay_input.split("/")
            ngay = f"{datetime.now().year}-{parts[1].zfill(2)}-{parts[0].zfill(2)}"
            c.execute("DELETE FROM chamcong WHERE ngay = ?", (ngay,))
            rows = c.rowcount
            conn.commit()
            conn.close()
            await update.message.reply_text(
                f"✅ Đã xóa *{rows}* bản ghi ngày *{ngay_input}*!",
                parse_mode="Markdown"
            )
        except:
            conn.close()
            await update.message.reply_text(
                "❌ Sai định dạng! Dùng: `/xoalichsu 07/06`",
                parse_mode="Markdown"
            )
...
def main():
    ...
    app.add_handler(CommandHandler("xoalichsu", xoa_lich_su))
